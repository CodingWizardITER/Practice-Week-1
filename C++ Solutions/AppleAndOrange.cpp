...
void countApplesAndOranges(int s, int t, int a, int b, int m, int n, vector<int> apples, vector<int> oranges) {
    int i, appleCount, orangeCount;
    // Setting counter variables to 0
    appleCount = orangeCount = 0;
    
    //checking for apples here
    for(i = 0; i < m; i++) {
        // If an apple falls between s and t then counter will be incremented
        if(a + apples[i] >= s && a + apples[i] <= t)
            appleCount++;
    }

    //checking for oranges here
    for(i = 0; i < n; i++) {
        // If an orange falls between s and t then counter will be incremented
